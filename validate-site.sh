...
# 👇 NEW: your site rewrites wp-admin to /piads
ADMIN_PATH="${ADMIN_PATH:-/piads/}"   # set to /wp-admin/ on sites without the rewrite
...
curl_check () {
  ...
  code=$(curl -sS -L -o /tmp/resp.html -w "%{http_code}" "$url" || true)
...
# ✅ Admin reachability (supports /piads); 200/302/403 are fine (403 = secured by plugin/firewall)
